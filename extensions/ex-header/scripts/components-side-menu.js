  ...
  $(document).ready(function() {
    ...
    $("[data-settings]").on("click", function() {
        ...
        if($(".settings-wrp").hasClass("settings-wrp--active")){
            if(rtl){
                $(".settings-wrp").css({"right" : "auto"}).animate({ "left" : '-' + (isMobile ? "100%" : $(".settings-wrp").outerWidth())}, 500 , closeSideBar).removeClass("settings-wrp--active").hide();
            }else {
                $(".settings-wrp").css({"left" : "auto"}).animate({ "right" : '-' + (isMobile ? "100%" :$(".settings-wrp").outerWidth())}, 500,closeSideBar).removeClass("settings-wrp--active").hide();
            }
            $(".ex-header__right-section__icon-wrp__overlay").hide();
        }else {
            if(rtl){
                $(".settings-wrp").show().css({"right" : "auto"}).animate({"left" : 0}, 500,closeSideBar).addClass("settings-wrp--active");
            }else {
                $(".settings-wrp").show().css({"left" : "auto"}).animate({"right" : 0}, 500,closeSideBar).addClass("settings-wrp--active");
            }
