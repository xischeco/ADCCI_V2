...
$(document).ready(function () {
  ...
  var highlightsSlick =
    $(highlightsSlickSelector) &&
    $(highlightsSlickSelector).data("slick") &&
    $(highlightsSlickSelector).slick("getSlick");

  if (
    $(baseClasshighlights).length &&
    !$(baseClasshighlights).hasClass(
      baseClasshighlights.replace(".", "") + "-v2"
    )
  ) {
    var isAutoPlay = !!(
        $(highlightsSlickSelector).data() &&
        $(highlightsSlickSelector).data().hasOwnProperty("autoplay") &&
        $(highlightsSlickSelector).data("autoplay") != "" &&
        !!$(highlightsSlickSelector).data("autoplay")
      ),
    ...
    var currentHolder = $(
        ".ex-image-highlights__navigation .slide-progress__current"
      ),
      countHolder = $(
        ".ex-image-highlights__navigation .slide-progress__count"
      ),
      progressBar = $(
    ...
    if (highlightsSlick && isAutoPlay && highlightsSlick.$slides.length > 1) {
      ...
      if (
        $(highlightsSlickSelector).data().hasOwnProperty("progress") &&
        ($(highlightsSlickSelector).data("progress") ||
          $(highlightsSlickSelector).data("progress") == "")
      ) {
      ...
      if (
        $(highlightsSlickSelector).data().hasOwnProperty("loader") &&
        ($(highlightsSlickSelector).data("loader") ||
          $(highlightsSlickSelector).data("loader") == "")
      ) {
        ...
        var isPause, percentTime, isLast;
    ...
    $(".highlights-scroll-down").click(function () {
      if (
        ...
        Scrollbar.get(document.querySelector("body")).scrollTo(
          0,
          $(highlightsSlickSelector).height() - $headerHeight,
          500
        );
      } else {
        $("html, body").animate(
          {
            scrollTop:
              $(highlightsSlickSelector).height() - (headerHeight - 25),
          },
          500
        );
      }
  ...
  function checkHeaderHeight(headerHeight) {
    if ($(".ex-header").length) {
      $(".ex-image-highlights-v2").animate(
        { paddingTop: headerHeight + "px" },
        800
      );
      $(".ex-inner-header").animate(
        { marginTop: headerHeight - 109 + "px" },
        800
      );
    }
  }
