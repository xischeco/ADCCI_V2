$(document).ready(function () {
  ...
  var inPageNvigationLinks = $('a[href*="#"]')
    .not('[href="#"]')
    .not('[href="#0"]');
  ...
  if (inPageNvigationLinks.length && !isTablet) {
    ...
    $(window).scroll(function () {
      ...
      const $scroll = $(".column-splitter");

      if ($scroll.length) {
        var scrolTop = $("html, body").scrollTop() - $scroll.offset().top;
        var topAmount = scrolTop + headerHeight;
        var deadEnd =
          leftNav.height() + topAmount > $(".column-splitter").height();
        if (!deadEnd) {
