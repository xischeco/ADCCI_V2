...
$(document).ready(function () {
    ...
    let dictionry = {
        ...
        "ar-ae" : {
            ...
            "lightmap" : "خريطة فاتحة",
            "darkmap" : "خريطة داكنة",
        },
        "he-il" : {
            "zoom" : "כדי להתקרב אנא לחץ פעמיים על המפה או השתמש בלחצני התקרבות / הרחבה ",
            "pan" : "אנא השתמש בשתי אצבעות כדי לסרוק את המפה",
            "fullExtent" : "היקף מלא",
            "lightmap" : "מפה קלה" ,
            "darkmap" : "מפה אפלה",
        }
    ...
    if(typeof require != "undefined" && tabContainer.length ){
        ...
        listContainer.find(".ex-accordion__item .ex-accordion__item__title").on("click",function(){
            var item = $(this).parent();
            
            if(view && !item.hasClass("expanded")){
                // remove from other items 
                listContainer.find(".ex-accordion__item").not(item).removeClass("expanded").find(".ex-accordion__item__content").slideUp();

                // show current item
                item.addClass("expanded").find(".ex-accordion__item__content").slideDown();

                var x = item.closest("li").data("x") , y = item.closest("li").data("y"), title = item.find(".ex-accordion__item__title").text().trim();
                // open popup and zoom 
                view.goTo({
                ...
                // close the item 
                item.removeClass("expanded").find(".ex-accordion__item__content").slideUp();
            ...
            if(isExEditor){
                listContainer.children("li").find(".ex-accordion__item__content").show();
            } else {
                listContainer.children("li:first-child").find(".ex-accordion__item__content").show();
            }
            ...
            function addPoints(){
                ...
                require([
                    ...
                    view.when(function(){
                        ...
                        $('.ex-map__container .ex-accordion__item:nth-child(1) .ex-accordion__item__title')[0].click();
                    }, function(error){
